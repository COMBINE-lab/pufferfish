...
enum class Rank : uint8_t {
    STRAIN, VARIETAS, 
    SUBSPECIES, SPECIES, SPECIESGROUP, SPECIESSUBGROUP, 
    SUBGENUS, GENUS, SUPERGENUS,
    SUBFAMILY, FAMILY, SUPERFAMILY, SUBTRIBE, TRIBE, 
    FORMA, COHORT, PARVORDER, 
    SUBORDER, ORDER, INFRAORDER, SUPERORDER, 
    SUBCLASS, CLASS, INFRACLASS, SUPERCLASS, 
    SUBPHYLUM, PHYLUM, SUPERPHYLUM, 
    KINGDOM, SUPERKINGDOM , DOMAINE, LIFE
};


class TaxaNode {
    public:
        TaxaNode() { rank = Rank::STRAIN; }
        TaxaNode(uint64_t inId, Rank inRank, uint64_t inPid) : id(inId), parentId(inPid), rank(inRank) {}
