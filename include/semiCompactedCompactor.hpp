...
private:
  ...
  // spp::sparse_hash_map<std::pair<std::string, bool>, std::pair<std::string,
  // bool>, cmpByPair > needsNewNext;
  std::map<std::pair<std::string, bool>, std::pair<std::string, bool>,
           util::cmpByPair>
      needsNewNext;
  ...
  bool is_boundary(std::vector<pufg::edgetuple>& inodes,
                   std::vector<pufg::edgetuple>& onodes, bool dirFlag) {
    if (dirFlag) { // realIndegree = 1 --> check for all real outgoings (
                   // outgoings from s+ and incomings to s- )
      for (auto& n : onodes) {
        if (n.baseSign) {
          if (pathStart.find(std::make_pair(n.contigId, n.neighborSign)) !=
                  pathStart.end() or
              pathEnd.find(std::make_pair(n.contigId, !n.neighborSign)) !=
                  pathEnd.end())
            return true;
        }
      }
      ...
      for (auto& n : inodes) {
        if (n.baseSign) {
          if (pathEnd.find(std::make_pair(n.contigId, n.neighborSign)) !=
                  pathEnd.end() or
              pathStart.find(std::make_pair(n.contigId, !n.neighborSign)) !=
                  pathStart.end())
            return true;
        }
      }
...
public:
  SemiCompactedCompactor(
      std::vector<std::pair<std::string, std::string>>& newSegmentsIn,
  ...
  void compact() {
    spp::sparse_hash_map<std::string, pufg::Node> vertices =
        semiGraph.getVertices();
    ...
    for (auto& idSeq : newSegments) {
      ...
      if (s.getRealIndeg() == 1 and s.getRealOutdeg() == 1 and
          is_boundary(s.getIn(), s.getOut(), true) and
          !is_boundary(s.getIn(), s.getOut(), false)) {
        clipIns = true;
      } else if (s.getRealIndeg() == 1 and s.getRealOutdeg() == 1 and
                 is_boundary(s.getIn(), s.getOut(), false) and
                 !is_boundary(s.getIn(), s.getOut(), true)) {
        clipOuts = true;
      }
      // It's either complex one2one with both outgoing and incoming edges being
      // boundary or the other cases
      else if ((s.getRealIndeg() > 1 and
                s.getRealOutdeg() > 1) //) it is a complex k-mer seg!! which
                                       //should be added to the path at the end
               or (s.getRealIndeg() == 1 and
                   is_boundary(s.getIn(), s.getOut(), true)) or
               (s.getRealOutdeg() == 1 and
                is_boundary(s.getIn(), s.getOut(), false))) {
        // std::cout<<s.getId()<<"\n";
        clipOuts = true;
        clipIns = true;
        ...
        for (auto& n : s.getIn()) { // keep all the incoming nodes to this node
                                    // in a map of the id to the new segment id
          needsNewNext[std::make_pair(n.contigId, n.neighborSign)] =
              std::make_pair(s.getId(), n.baseSign);
          if (pathStart.find(std::make_pair(n.contigId, !n.neighborSign)) !=
              pathStart.end()) {
            needsNewPrev[std::make_pair(n.contigId, !n.neighborSign)] =
                std::make_pair(s.getId(), !n.baseSign);
          }
        }
        for (auto& n : s.getOut()) {
      ...
      } else if (s.getRealIndeg() == 1)
        clipOuts = true;
      else if (s.getRealOutdeg() == 1)
        clipIns = true;
      /*
      if ( (s.getRealIndeg() == 1 and is_boundary(s.getOut())) or
      (s.getRealOutdeg() == 1 and is_boundary(s.getIn())) ) {
          boundaryCaseCntr++;
      }*/
      /*
       * 0 ---> 1
       ...
       * 0 ---> 3
       */
      if (clipOuts) { // just clip off from s+ outgoing nodes and s- incoming
                      // nodes
        for (auto& n : s.getOut()) { // outgoing nodes
          if (n.baseSign) {          // outgoing nodes from s+
            if (n.neighborSign and
                left_clipped.find(n.contigId) ==
                    left_clipped.end()) { // s+ --> n+
              contigid2seq[n.contigId].erase(0, 1);
              left_clipped[n.contigId] = true;
            } else if (!n.neighborSign and
                       right_clipped.find(n.contigId) ==
                           right_clipped.end()) { // s+ --> n-
              contigid2seq[n.contigId].erase(
                  contigid2seq[n.contigId].size() - 1, 1); // last nucleotide
        ...
        for (auto& n : s.getIn()) { // incoming nodes
          if (!n.baseSign) {        // incoming nodes to s-
            if (n.neighborSign and
                right_clipped.find(n.contigId) ==
                    right_clipped.end()) { // n+ --> s-
              contigid2seq[n.contigId].erase(
                  contigid2seq[n.contigId].size() - 1, 1); // last nucleotide
              right_clipped[n.contigId] = true;
            } else if (!n.neighborSign and
                       left_clipped.find(n.contigId) ==
                           left_clipped.end()) {    // n- --> s-
       ...
       * 3 --> 0
       */
      if (clipIns) { // just clip off from s+ incoming nodes and s- outgoing
                     // nodes
        for (auto& n : s.getIn()) { // incoming nodes
          if (n.baseSign) {         // incoming nodes to s+
            if (n.neighborSign and
  ...
  void updatePath(
      spp::sparse_hash_map<std::string,
                           std::vector<std::pair<std::string, bool>>>& pathIn) {
    path = pathIn;
    for (auto& kv : path) {
