...
for idx in "${!names[@]}"; do
	...
	echo "Puffaligner"
	name="${root}/alignments/puffaligner/${dataType}.pe.concordant.${names[$idx]}"
	cmd="/usr/bin/time /home/mohsen/Pufferfish-repo/pufferfish/build/src/pufferfish align -i ${root}/indices/pufferfishindices/microbiome_k31.resubmission.puffindex.sparse/ -1 ${r1} -2 ${r2} -o ${name}.sam  --noOrphans --noDiscordant ${puffOpts[$idx]} -t 16 > ${name}.log 2>&1"
	echo ${cmd}
#eval ${cmd}
#	echo ${cmd} >> ${name}.log
	echo ""
