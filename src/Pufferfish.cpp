...
int main(int argc, char* argv[]) {
  ...
  */
  ...
  auto alignMode = (
                    ...
                    (option("--computeLM").set(alignmentOpt.computeLM, true)) % "Compute the length of the longest match for each alingnment, using this flag sets the recoverOrphan flag to false",
                    ...
                    (option("--match") & value("match score", alignmentOpt.matchScore)) % "match score for each base",
                    (option("--mismatch") & value("mismatch score", alignmentOpt.missMatchScore)) % "mismatch score for each base (should be negative)",
                    (option("--gapo") & value("gap open penaly", alignmentOpt.gapOpenPenalty)) % "penalty for openning a gap (insertion or deletion)",
                    (option("--gape") & value("gap extend penalty", alignmentOpt.gapExtendPenalty)) % "Penalty for extending a gap by each base",
