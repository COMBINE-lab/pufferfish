...
int main(int argc, char* argv[]){
		...
		std::string prefix = "00";//TODO we just keep it number to pass is_number, but remember not to EVER convert contigID to number (e.g. uint64_t) b/c it'll have conflict of IDs
              ...
              if (contigid2seq[curr].length() <= k+1 or contigid2seq[next].length() <= k+1)  { continue; }
							...
							if (contigVec[i].second) {// left +
								if (right_clipped.find(contigVec[i].first) == right_clipped.end()) {
									contigid2seq[contigVec[i].first].erase(contigid2seq[contigVec[i].first].size()-1, 1); // erase right most nucleotide
									right_clipped[contigVec[i].first] = true;
								}
							}
							else {// left -
								if (left_clipped.find(contigVec[i].first) == left_clipped.end()) {
									contigid2seq[contigVec[i].first].erase(0, 1); // erase left most nucleotide
									left_clipped[contigVec[i].first] = true;
								}							
							}
							if (contigVec[i+1].second) {// right +
								if (left_clipped.find(contigVec[i+1].first) == left_clipped.end()) {
									contigid2seq[contigVec[i+1].first].erase(0, 1); // erase left most nucleotide
									left_clipped[contigVec[i+1].first] = true;
								}
							}
							else {// right -
								if (right_clipped.find(contigVec[i+1].first) == right_clipped.end()) {
									contigid2seq[contigVec[i+1].first].erase(contigid2seq[contigVec[i+1].first].size()-1, 1); // erase right most nucleotide
							...
							if (kmer2contigid.find(kmer) != kmer2contigid.end()){
									...
									updatedContigVec.emplace_back(kmerId, contigVec[i].second); // insert the kmer segment with the same orientation as left segment
							} else if (kmer2contigid.find(util::revcomp(kmer)) != kmer2contigid.end()){
									kmerId = kmer2contigid[util::revcomp(kmer)] ;
									updatedContigVec.emplace_back(kmerId, !contigVec[i].second);  // insert the kmer segment with the REVERSE orientation as left segment
							} else {
					...
					} else updatedContigVec.push_back(contigVec[0]);
